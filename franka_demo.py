...
class IsaacSim():
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        asset_file_kit = "urdf/kitchen/coffee/coffee.urdf"

        asset_options = gymapi.AssetOptions()
        asset_options.fix_base_link = True
        coffee_asset = self.gym.load_asset(self.sim, asset_root_kit, asset_file_kit, asset_options)

        # create urdf asset
        asset_root_urdf = "../assets"
        asset_file_urdf = "urdf/kitchen/48452/mobility.urdf"

        asset_options = gymapi.AssetOptions()
        asset_options.fix_base_link = True
        urdf_asset = self.gym.load_asset(self.sim, asset_root_urdf, asset_file_urdf, asset_options)
        urdf_dof_names = self.gym.get_asset_dof_names(urdf_asset)
        self.num_dofs += self.gym.get_asset_dof_count(urdf_asset)
        ...
        for i in range(num_envs):
            ...
            # create urdf and set properties
            urdf_handle = self.gym.create_actor(env_ptr, urdf_asset, urdf_start_pose, "urdf", i, 8, 3)
            urdf_sim_index = self.gym.get_actor_index(env_ptr, urdf_handle, gymapi.DOMAIN_SIM)
            self.urdf_indices.append(urdf_sim_index)

            # urdf_dof_index = [
            #     self.gym.find_actor_dof_index(env_ptr, urdf_handle, dof_name, gymapi.DOMAIN_SIM)
            #     for dof_name in urdf_dof_names
            # ]

            urdf_link_sim_idx = self.gym.find_actor_rigid_body_index(env_ptr, urdf_handle, "link_0", gymapi.DOMAIN_SIM)
            self.urdf_link_indices.append(urdf_link_sim_idx)

            # self.gym.set_actor_dof_properties(env_ptr, urdf_handle, urdf_dof_props)
            self.gym.set_actor_scale(env_ptr, urdf_handle, 0.3)
    ...
    def reset(self):
        self.franka_init_pose = np.array([-0.6235, -0.4089,  0.0705, -2.7245,  0.0327,  2.3281,  0.1783,  0.0400, 0.0400])
